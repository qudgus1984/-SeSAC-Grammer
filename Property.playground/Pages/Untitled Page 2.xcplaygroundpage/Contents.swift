...
struct BMI {
    var nickname: String {
        willSet {
            print("유저 닉네임이 \(nickname)에서 \(newValue)로 변경될 예정이에여")
        }
        didSet {
            print("닉네임이 \(oldValue)에서 \(nickname)으로 변경되었어영")
        }
    }
    var weight: Double
    var height: Double
...
var bmi = BMI(nickname: "고래밥", weight: 63, height: 175)
...
// 열거형은 타입 자체 > 인스턴스 생성이 불가능함 > 초기화 구문 X
...
// 인스턴스 저장 프로퍼티는 메모리에, 값이 달라질 수 있음 > 열거형 사용 불가능 > 열거형은 초기화 구문을 만들 수 없기 때문
...
TypeFoodRestaurant.nowOrder // 연산 타입 프로퍼티 Get
...
//class Latte: Coffee {
//    override class func minusShot() { // 슈퍼 클래스의 타입 메서드를 재정의해서 쓰고싶다면 class
//        <#code#>
//    }
//}
